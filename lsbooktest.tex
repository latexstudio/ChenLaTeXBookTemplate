% !TeX document-id = {96b7c153-3a28-4e93-b6cb-e11439734af4}
...
\mainmatter
\chapter{模板使用说明}
\mybox{关于代码排版盒子中使用listings和minted的说明}

由于使用minted对于初学者来说比较难以配置，所以，这里根据用户编译选项中是否使用\\
\verb|--shell-escape|来自动切换使用listings还是minted。不用担心，命令使用上没有任何不同，只是输出效果上有差别。
...
\begin{tcblisting}{listing only,applelight,title={自己来设定颜色吧！},
  listing options={numbers=none,basicstyle=\ttfamily,columns=fullflexible,
...
\langCVfile[代码语言][盒子标签][语言名显示]{标题}{文件名} # 中文注释
\gitfile[代码语言]{标题}{文件名}
